...
def STOCHRSI(series, timeperiod=14, smoothK=3, smoothD=3):
    # Calculate RSI
    ...
    ups[ups.index[timeperiod-1]] = np.mean(ups[:timeperiod])
    ups = ups.drop(ups.index[:(timeperiod-1)])
    # first value is sum of avg losses
    downs[downs.index[timeperiod-1]] = np.mean(downs[:timeperiod])
    downs = downs.drop(downs.index[:(timeperiod-1)])
    rs = ups.ewm(com=timeperiod-1, min_periods=0, adjust=False, ignore_na=False).mean() / \
        downs.ewm(com=timeperiod-1, min_periods=0,
                  adjust=False, ignore_na=False).mean()
    rsi = 100 - 100 / (1 + rs)

    # Calculate StochRSI
    stochrsi = (rsi - rsi.rolling(timeperiod).min()) / \
        (rsi.rolling(timeperiod).max() - rsi.rolling(timeperiod).min())
    stochrsi_K = stochrsi.rolling(smoothK).mean()*100
    # stochrsi_D = stochrsi_K.rolling(smoothD).mean()

    return stochrsi_K.to_numpy()
